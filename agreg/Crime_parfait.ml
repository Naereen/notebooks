...
(* ---- *)
(* ## Implémentation *)
(* La question d'implémentation était la question 2) en page 7.# > « Proposer une structure de donnée adaptée pour représenter un graphe d'intervalles dont une représentation sous forme de famille d’intervalles est connue. *)
(* > Implémenter de manière efficace l’algorithme de coloriage de graphes d'intervalles et illustrer cet algorithme sur une application bien choisie citée dans le texte. »# Nous allons donc d'abord définir une structure de donnée pour une famille d'intervalles ainsi que pour un graphe d'intervalle, ainsi qu'une fonction convertissant l'un en l'autre.# Cela permettra de facilement définr les différents exemples du texte, et de les résoudre. *)

(* ### Une bonne structure de donnée pour des intervalles et des graphes d'intervales# - Pour des **intervalles** à valeurs réelles, on se restreint par convénience à des valeurs entières. *)
...
(* ### Algorithme de coloriage de graphe d'intervalles# > Étant donné un graphe $G = (V, E)$, on cherche un entier $n$ minimal et une fonction $c : V \to \{1, \cdots, n\}$ telle que si $(v_1 , v_2) \in E$, alors $c(v_1) \neq c(v_2)$.# On suit les indications de l'énoncé pour implémenter facilement cet algorithme.# > Une *heuristique* simple pour résoudre ce problème consiste à appliquer l’algorithme glouton suivant : *)
(* > - tant qu'il reste reste des sommets non coloriés, *)
(* >    + en choisir un *)
(* >    + et le colorier avec le plus petit entier qui n’apparait pas dans les voisins déjà coloriés.# > En choisissant bien le nouveau sommet à colorier à chaque fois, cette heuristique se révelle optimale pour les graphes d’intervalles. *)
...
(* | 6 | Le colonel Ferdinand                   | 20h .. 20h59    | $1200 \cdots 1259$# > Combien de tables le chef de train doit-il prévoir ? *)

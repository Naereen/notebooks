...
(*  - *Texte*: Annale 2012, ["Tissus cellulaires" (public2010-D2)](http://agreg.org/Textes/public2010-D2.pdf) *)
...
(*  - Cette fonction s'exécute en temps linéaire $\mathcal{O}(n \times m)$, puisqu'elle appelle $m$ fois `une_etape_infection` qui est en temps linéaire $\mathcal{O}(n)$. *)
...
(*  ---- *)
(*  ## Complexités en temps et espace *)

(*  ### En temps *)
(*  - Une étape d'infection, i.e., `une_etape_infection`, est en temps linéaire $\mathcal{O}(n)$, *)
(*  - Donc $m$ étapes, i.e., `m_etapes_infection`, est en temps linéaire $\mathcal{O}(n \times m)$. *)

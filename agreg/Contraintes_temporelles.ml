...
(* La question de programmation pour ce texte était donnée en question 1 en page 9 :# > "Programmer l'algorithme PC. Faites le tourner sur l'exemple de réseau non distributif."# La consigne était très courte, mais avec aucune indication. *)
(* Notez qu'il est rare que le texte exige un exemple particulier. *)

(* ---- *)
(* ## Réponse à l'exercice requis *)
(* Ça va être assez rapide :# 1. D'abord, il faut définir les types de données avec lesquels on va travailler, *)
...
(* 4. Et enfin, l'exécuter sur l'exemple de réseau non distributif donné en page 8.# Si possible, on va essayer de faire des *tests* pour chaque fonction intermédiaire, et un exemple de plus à la fin. *)

(* ---- *)
(* ### Structures de données# On se restreint aux intervalles à coordonnées *entières*, et on considère des listes d'intervalles. *)
...
(* On peut écrire des opérations d'intersection et de composition sur deux intervalles, ensuite il suffira de les généraliser à un ensemble d'intervalle.# On suit les définitions de l'énoncé.# #### Intersection *)
(* $$ *)
...
(* On supprimera les doublons en vérifiant au fur et à mesure (ça a la même complexité que si on le fait à la fin).# En manipulant une liste d'intervalle `option`, on doit ruser un peu pour n'ajouter que ceux qui ne sont pas dans `acc` et qui sont des vrais intervalles. *)
...
(* Avec tout ça, on a une belle fonction récursive, avec un accumulateur `acc` qui contient la liste des intervalles dans $T \oplus S$, construite en considérant les intervalles de $S$ les un après les autres.# On s'assure de n'avoir ni intervalles vide, ni doublon, grâce à `ajoute_nouveaux_option`. *)
...
(* On retrouve bien le résultat de la Figure 2.b).# L'intervalle $[3, 4]$ est inclus dans $[2, 5]$, donc on devrait ajouter une étape de nettoyage pour donner une forme canonique aux intervalles produit par `composition`. On le fait plus bas. *)
...
(* #### Une étape de plus : ne pas inclure des intervalles inclus dans ceux déjà présents# On va raffiner les fonctions définis ci-dessus en ajoutant un test, sur leur résultat final.# - `est_inclus i j` teste si $I \subseteq J$. *)
...
(* Notez que pour obtenir une vraie forme canonique, il faudrait aussi rassembler les intervalles consécutifs ($\{ [0, 1], [1, 2] \} \rightarrow \{ [0, 1] \}$) et se recoupant ($\{ [0, 3], [2, 4] \} \rightarrow \{ [0, 4] \}$).# > Ça prendrait trop de temps. Et ce n'était pas exigé. *)
...
(* En simplifiant, on obtient :# - $T_{01} \otimes (T_{13} \oplus S_{13} = \{ [25, 31], [35, 70] \}$, *)
(* - $(T_{01} \otimes T_{13}) \oplus (T_{01} \otimes S_{13}) = \{ [25, 70] \}$,# qui sont bien différents. *)
...
(* Maintenant, passons à l'algorithme PC.# ![images/algorithme_PC.png](images/algorithme_PC.png). *)
...
(* On va traiter l'exemple de la Figure 4 du texte, comme défini plus haut :# ![images/reseau_distributif.png](images/reseau_distributif.png) *)
...
(* ---- *)
(* ### D'autres exemples# On peut étudier le STP de la Figure 1., en enlevant la contrainte $[60, \infty)$, qui ne rentre pas dans notre implémentation.# ![images/stp_figure1.png](images/stp_figure1.png). *)
...
(* ---- *)
(* ## Conclusion# Voilà pour la question obligatoire de programmation :# - on a décomposé le problème en sous-fonctions, *)
(* - on a essayé d'être fainéant, en réutilisant les sous-fonctions, *)
(* - on a fait des exemples et *on les garde* dans ce qu'on présente au jury, *)
(* - on a testé la fonction exigée sur de petits exemples et sur un exemple de taille réelle (venant du texte)# Et on a essayé de faire *un peu plus*, en implémentant la vérification d'une contrainte de plus.# > Bien-sûr, ce petit notebook ne se prétend pas être une solution optimale, ni exhaustive. *)
